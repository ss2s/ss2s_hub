...
bool flag_cooler_on_timer = 0;  // вентиляция таймер
...
void displaySetup()
{
	pinMode(DEF_LCD_LIGHT_PIN, OUTPUT);
...
void checkTableParam()
{
	...
	table_vent_on = int(basic_schedule[day_control_val][2]);
	table_vent_off = int(basic_schedule[day_control_val][3]);
}
...
void ventControl()
{
	...
	if(curent_temp > (table_temp + cooling_histerezis)){
		...
		if((flag_cooler_on == 1 && flag_cooler_on_overheating == 1) 
			|| (flag_cooler_on_timer == 1 && (millis() - start_timer_1) >= (table_vent_on * 1000*60))){
		...
		else if((flag_cooler_on_timer == 0 && (millis() - start_timer_1) >= (table_vent_off * 1000*60))){
			digitalWrite(RELE_COOLER_PIN, RELE_HIGH);
			flag_cooler_on = 1;
			flag_cooler_on_overheating = 0;
		    flag_cooler_on_timer = 1;
