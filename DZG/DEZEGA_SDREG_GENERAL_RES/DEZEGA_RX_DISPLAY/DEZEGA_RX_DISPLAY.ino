...
void recbuttonChangeCheckGF(){
	if(digitalRead(START_STOP_BUTTON_PIN) == BUTTON_TYPE){  // если кнопка нажата
		delay(50);
		if(digitalRead(START_STOP_BUTTON_PIN) == BUTTON_TYPE){
			// Serial.println("but pres");
...
void setup(){
	...
	if(BUTTON_TYPE == 1){pinMode(START_STOP_BUTTON_PIN, INPUT);}
	else if(BUTTON_TYPE == 0){pinMode(START_STOP_BUTTON_PIN, INPUT_PULLUP);}
	else {
		while(1){
		    iLed[0] = CRGB(255, 255, 255);  // GRB white
			FastLED.show();
			delay(1000);
			iLed[0] = CRGB(0, 0, 0);  // GRB white
			FastLED.show();
			delay(1000);
		}
	}
