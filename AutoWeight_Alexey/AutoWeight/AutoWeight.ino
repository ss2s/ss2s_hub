...
#define SONAR_COEF 5.7
#define VOLUME_COEF 1000
...
String command = "";
String tempString = "";
volatile float arrayOfSonarsValues[4] = {0};
...
float nWeightF(){
  ...
  if (Serial2.available()) {
    ...
    for(int i=0; i<10; i++){
      delay(5);
      rs232_data = Serial2.parseFloat();
      if(rs232_data == '='){
        break;
      }
    }
    // rs232_data = Serial2.parseFloat();
    tWeight = Serial2.parseFloat();
    Serial.println("");
    Serial.println(tWeight,3);
    Serial.println("");

    if(tWeight !=0){return tWeight;}
    else{return 0.000;}
  }
  else{return 0.000;}
...
class Tenzo {
  public:
    float scaleUnits = 0.004;
    float rationGrams = 0.03527;
    float weight = 0.005;
    float caliboration = 1.000;
    float tempCaliboration  = 1.000;
    ...
    void getUnits ()  {
      ...
      if (scaleUnits < 0) {weight = 0.000;}
        else if(isnan(scaleUnits)) {weight = 0.000;}
        else {
...
void setup() {
  ...
  Serial2.begin(9600); // init UART rs232
  mySerial.begin(9600); // init Software UART rs232
...
void menuShowOnLCDData()  {
  ...
  while (menuPage == 1) {
    ...
    if (currentMillis - previousMillis > 1000) {
      ...
      Box1.length = float(1000.0 - sonya1.pingFloat / SONAR_COEF )  / 10.0;

      Box1.height = float(1000.0 - sonya2.pingFloat / SONAR_COEF )  / 10.0;

      Box1.width  = float(1000.0 - sonya3.pingFloat / SONAR_COEF )   / 10.0;
      ...
      float floatvar  = float(total) / VOLUME_COEF;
      ...
      tenzoStr = String(tenzo.weight, 3);
...
void menuPageVersion()  {
  ...
  while (menuPage == 2)  {
    ...
    if (currentMillis - previousMillis > 900) {
      ...
      //  D1
      lcd.setCursor(2, 1);
      lcd << sonya1.pingFloat / SONAR_COEF << "   ";
      lcd.setCursor(12, 1);
      lcd << sonya1.pingFloat / SONAR_COEF << "  ";
      ...
      //  D2
      lcd.setCursor(2, 2);
      lcd << sonya2.pingFloat / SONAR_COEF  << "  ";
      ...
      // D3
      lcd.setCursor(2, 3);
      lcd << sonya3.pingFloat / SONAR_COEF << "   ";
...
void insertSonarsValues (String tempString) {
  for (int i = 1; i <= 3; i++) {
    ...
    Serial << strTemp << ' ';
    if (valueForBTWeight = strTemp.toFloat()) {  // WTF_
      valueForBTWeight = strTemp.toFloat();
      arrayOfSonarsValues[i] = valueForBTWeight;
    }
  }
}
void saveIntoEEPROMSonarsValues () {  // sonar calibration correction _
  ...
  settings.Vl1                = float(settings.EEPROM_float_read(4));
  settings.Vl2                = float(settings.EEPROM_float_read(8));
  settings.Vl3                = float(settings.EEPROM_float_read(16));
