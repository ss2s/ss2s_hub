...
#define HEIGHT 9        // высота матрицы (число диодов)
#define BRIGHTNESS 200    // яркость (0 - 255)

// цвета высоты полос спектра. Длины полос задаются примерно в строке 135
// #define COLOR1 CRGB::Blue
// #define COLOR2 CRGB::Blue
// #define COLOR3 CRGB::Blue
...
#define DELAY 1           // задержка между обновлениями матрицы (периодичность основного цикла), миллиисекунды
...
#define MAX_COLOR CRGB::Blue // цвет точек максимума
...
// byte posOffset[19] = {2, 3, 4, 6, 8, 10, 12, 14, 16, 20, 25, 30, 35, 45, 55, 65, 80, 100, 120};
// -------------------------------- НАСТРОЙКИ --------------------------------
    ...
    for (byte pos = 0; pos < WIDTH; pos++) {    // для кажого столбца матрицы
      ...
      if (posLevel > 0) {
        for (int j = 0; j < posLevel; j++) {                 // столбцы
          uint32_t color;
          if (j < 2) color = COLOR1;
          else if (j < 4) color = COLOR2;
          else if (j < 6) color = COLOR3;
          else if (j < 8) color = COLOR4;

          leds[pos * WIDTH + j] = color;                  // заливаем в прямом порядке SS
...
/*
   ...
   Переход от 128 полос к 18 полосам с сохранением межполосных значений по линейной зависимости
